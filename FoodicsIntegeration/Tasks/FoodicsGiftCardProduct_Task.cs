...
{
    public class FoodicsGiftCardProduct_Task : Foodics_BaseIntegration
    {
        public override void Get(string Subsidiary)
        {
           
            var client = new RestClient(ConfigurationManager.AppSettings["Foodics.ResetURL"] + "gift_card_products");
            client.Timeout = -1;
            var request = new RestRequest(Method.GET);
            request.AddHeader("Authorization", "Bearer " + ConfigurationManager.AppSettings["Foodics.Token"]);
            var response = client.Execute<Dictionary<string, List<FoodicsGiftCardProduct>>>(request);
                ...
                foreach (var item in response.Data)
                {
                    if (item.Key == "data")
                    {
                        ...
                        if (item.Value.Count > 0)
                            //new GenericeDAO<FoodicsGiftCardProduct>().FoodicsIntegration(item.Value);
